...
// مفتاح الـ Cache يتضمن userId لضمان عزل البيانات بين المستخدمين
const getUsersCacheKey = (userId: string) => `users_data_${userId}`;
...
const PermissionManagement: React.FC<PermissionManagementProps> = ({ className = "" }) => {
  ...
  const cacheKey = currentUser?.id ? getUsersCacheKey(currentUser.id.toString()) : null;
  ...
  // States for API data - لا نقرأ من الـ Cache في البداية لضمان جلب البيانات الصحيحة
  ...
  // مسح الـ Cache القديم من المستخدمين السابقين عند تغيير المستخدم
  useEffect(() => {
    // مسح جميع cache المستخدمين القدامى
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('users_data') && key !== cacheKey) {
  ...
  const loadUsers = async (filters: UserFilters = {}, forceRefresh = false) => {
    ...
    if (!forceRefresh && !searchTerm && selectedRole === 'all' && selectedStatus === 'all' && cacheKey) {
    ...
    try {
      ...
      // Save to cache (only if default filters and cacheKey exists)
      if (!searchTerm && selectedRole === 'all' && selectedStatus === 'all' && cacheKey) {
  ...
  useEffect(() => {
    // Always fetch fresh data - no cache check on mount
    loadUsers(undefined, true); // forceRefresh = true
  }, [searchTerm, selectedRole, selectedStatus, currentPage, currentUser?.id]);

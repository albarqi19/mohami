...
const WhatsappSettings: React.FC = () => {
  ...
  const loadInstances = async () => {
    try {
      console.log('📱 تحميل instances من الباك إند...');
      ...
      if (response.data.success) {
        ...
        console.log('✅ تم تحميل', response.data.data.length, 'instances');
      }
    } catch (error) {
  ...
  const createInstance = async () => {
    ...
    try {
      console.log('🚀 إنشاء instance جديد عبر الباك إند...');
      ...
      if (!response.data.success) {
        throw new Error(response.data.message || 'فشل إنشاء الـ Instance');
      }

      console.log('✅ تم إنشاء الـ Instance:', response.data.data);

      const newInstance: WhatsappInstance = {
        id: response.data.data.id.toString(),
        instance_name: response.data.data.instance_name,
      ...
      // جلب QR Code بعد الإنشاء
      console.log('⏳ جلب QR Code...');
      setTimeout(async () => {
      ...
      const checkStatus = setInterval(async () => {
        try {
          const statusResponse = await api.get(`/v1/whatsapp/instances/${newInstance.id}/status`);

          if (statusResponse.data.success) {
            const status = statusResponse.data.data.status;
            const phoneNumber = statusResponse.data.data.phone_number;

            console.log('📱 حالة Instance:', status, phoneNumber);
            ...
            if (status === 'connected') {
              ...
              setInstances(prev => prev.map(instance =>
                instance.id === newInstance.id
                  ? { ...instance, status: 'connected', phone_number: phoneNumber }
                  : instance
              ));
      ...
      // إيقاف الفحص بعد 90 ثانية
      setTimeout(() => clearInterval(checkStatus), 90000);

    } catch (error) {
      console.error('❌ خطأ في إنشاء الـ Instance:', error);
      alert(`خطأ في إنشاء الـ Instance: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
    }
  };
  ...
  const deleteInstance = async (instanceId: string) => {
    ...
    try {
      console.log('🗑️ حذف instance:', instanceId);

      const response = await api.delete(`/v1/whatsapp/instances/${instanceId}`);

      if (response.data.success) {
        setInstances(prev => prev.filter(inst => inst.id !== instanceId));
        console.log('✅ تم حذف الـ Instance بنجاح');
      } else {
        throw new Error(response.data.message || 'فشل في الحذف');
      }

    } catch (error) {
      console.error('❌ خطأ في حذف instance:', error);
      alert(`خطأ في حذف الـ Instance: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
    }
  };

  const getQRCode = async (instanceId: string) => {
    try {
      console.log('📱 جلب QR Code للـ instance:', instanceId);

      const response = await api.get(`/v1/whatsapp/instances/${instanceId}/qrcode`);

      if (response.data.success) {
        const qrData = response.data.data.qr_code;

        if (qrData) {
          console.log('✅ تم جلب QR Code');

          if (typeof qrData === 'string') {
            if (qrData.startsWith('data:image') || qrData.startsWith('http')) {
              setSelectedQRCode(qrData);
            } else {
              // نص QR - تحويله باستخدام خدمة خارجية
          ...
          console.log('⚠️ لم يتم العثور على QR Code');
        }
      } else {
      ...
      alert(`خطأ في جلب QR Code: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`);
    }
  };

  // اختبار الاتصال مع الباك إند
  const testEvolutionAPI = async () => {
    try {
      ...
      if (response.data.success) {
        console.log('✅ الباك إند متاح');
        alert('✅ الاتصال يعمل بشكل صحيح!');
      } else {
        throw new Error('الباك إند غير متاح');
      }
    } catch (error) {
      console.error('❌ خطأ في الاتصال:', error);
      alert('❌ فشل الاتصال. تأكد من تشغيل الباك إند');
    }
  };
